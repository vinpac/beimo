...
export default async function parseDefaults({ sourcePath }) {
  ...
  filesToParse.forEach(async filepath => {
    let body = await readFile(path.join(__dirname, '../defaults', filepath))
    ...
    testNodes.forEach(node => {
      ...
      body = body.replace(
        node.reImport,
        () => {
          if (node.serverOnly && /client\.js$/.test(filepath)) {
            throw new Error(`${node.key} are server-only`)
          }

          return node.condition
            ? `import ${node.key} from '${path.join(sourcePath, node.path)}'`
      ...
      body = body.replace(
        ...
        () => {
          ...
          return `'${path.join(sourcePath, node.path)}'`
        },
      )
    })

    await writeFile(path.join(__dirname, '../.beimo', filepath), body)
  })
